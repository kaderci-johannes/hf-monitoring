...
DATA = []
histos = [[TH1D("HF%s0%i_%s" %(pm,crate,hname.lower().replace(' ','_')),"HF%s0%i %s" %(pm,crate,hname),60*t/dt,-dt/120.,t-dt/120.) for hname in hnames] for pm in ['P','M'] for crate in crates]
htitles=["HF%s0%i %s" %(pm,crate,hname) for pm in ['P','M'] for crate in crates for hname in hnames]
cmds = ["get HF%s0%i-%s" %(pm,crate,reg) for pm in ['P','M'] for crate in crates for reg in regs]

histos.append([TH1D("HFM08_%s" %(hname.lower().replace(' ','_')),"HF%s0%i %s" %(pm,crate,hname),60*t/dt,-dt/120.,t-dt/120.) for hname in hnames])
htitles+=["HFM08 %s" %(hname) for hname in hnames]
cmds+=["get HFM08-%s" %reg for reg in regs]
